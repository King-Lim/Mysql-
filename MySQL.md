...
```bash
#系统变量，指定跳过复制事件的个数N
mysql> set global sql_slave_skip_counter = 1 | N;
...
# group by
select classid,max(age) from students group by classid having classid = 1;			# group by后接having，语法要求
select classid,max(age) from students where classid = 1 group by classid;
```
...
# 联合查询：UNION	（上下合表，保证表格字段数、数据类型一致）
...
# 自连接：本表和本表进行连接查询
select A.name,B.name from emp as A left join emp as B on A.id = B.leaderid;
```
